...
curl -L -o /tmp/image2WebP.zip "https://github.com/TheBluWiz/image2WebP/archive/refs/tags/v0.2.0.zip" || { echo "Failed to download the script"; exit 1; }
unzip /tmp/image2WebP.zip -d /tmp || { echo "Failed to unzip the script"; exit 1; }

# Copy the script to ~/.bin and make it executable
cp /tmp/image2WebP-0.1.0/image2WebP.sh ~/.bin/image2WebP || { echo "Failed to copy the script to ~/.bin"; exit 1; }
